...
for (let i = 1; i <= 300; i++) {
  ...
  if (i === 12) { status = 'FAIL'; severity = 'High'; }
  if (i === 45) { status = 'FAIL'; severity = 'Medium'; }
...
// Make a copy for endpoint-inventory as requested by user
xlsx.writeFile(workbook, path.join(OUTPUT_DIR, 'endpoint-inventory.xlsx'));
